...
def decode_vin_full(vin: str, timeout: float = 3.0) -> Dict[str, str]:
    ...
    if "engine_desc" not in result:
        ...
        if result.get("cylinders"):
            ...
            config = result.get("engine_configuration", "")
            if "v-shaped" in config.lower() or "v shaped" in config.lower():
                parts.append(f"V{cyl}")
            elif "in-line" in config.lower() or "inline" in config.lower():
                parts.append(f"I{cyl}")
            elif "horizontally" in config.lower() or "flat" in config.lower():
                parts.append(f"H{cyl}")
            elif int(cyl) >= 5:
                parts.append(f"V{cyl}")
            elif int(cyl) <= 4:
                parts.append(f"I{cyl}")
        ...
        if result.get("valve_train"):
            vt = result["valve_train"]
            # Shorten to acronym if it contains one in parens
            if "DOHC" in vt:
                parts.append("DOHC")
            elif "SOHC" in vt:
                parts.append("SOHC")
                ...
                parts.append(vt)
        if result.get("turbo") and result["turbo"].lower() == "yes":
            parts.append("Turbo")
