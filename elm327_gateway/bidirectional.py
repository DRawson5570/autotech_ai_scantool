...
def _load_actuator_dids() -> List[Dict[str, Any]]:
    """Load verified actuator DIDs from scan_tool_data.db."""
    import json
    import os
    import sqlite3
    ...
    if not os.path.exists(db_path):
        logger.warning("scan_tool_data.db not found — actuator DIDs empty")
        return []
    ...
    try:
        ...
        for row in conn.execute("SELECT * FROM actuators ORDER BY id"):
            result.append({
                "manufacturer": row["manufacturer"],
                "module": row["module"],
                "module_addr": row["module_addr"],
                "bus": row["bus"],
                "security_required": bool(row["security_required"]),
                "signal_name": row["signal_name"],
                "did_hex": row["did_hex"],
                "control_param": row["control_param"],
                "on_data": bytes(row["on_data"]) if row["on_data"] else b"",
                "off_data": bytes(row["off_data"]) if row["off_data"] else b"",
                "caution_level": row["caution_level"],
                "description": row["description"],
            })
        conn.close()
        logger.info("Loaded %d actuator DIDs from database", len(result))
    except Exception as e:
        logger.error("Failed to load actuator DIDs from DB: %s", e)
    return result


# Load at import time
VERIFIED_ACTUATOR_DIDS = _load_actuator_dids()
